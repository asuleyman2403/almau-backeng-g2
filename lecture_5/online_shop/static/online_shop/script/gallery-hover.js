...
for(let i = 0; i < 6; i++){
    let number = i + 1;
    galleryItems[i].style.background = `url('${DJANGO_STATIC_URL}online_shop/images/work-${number}.jpg') no-repeat center`;
    ...
    galleryItems[i].addEventListener('mouseover',function () {
            ...
            galleryItems[i].style.background = `linear-gradient(rgba(238, 238, 238,.7), rgba(238, 238, 238, .7)), url('${DJANGO_STATIC_URL}online_shop/images/work-${number}.jpg') no-repeat center`;
            galleryItems[i].style.backgroundSize = "cover";
    });
